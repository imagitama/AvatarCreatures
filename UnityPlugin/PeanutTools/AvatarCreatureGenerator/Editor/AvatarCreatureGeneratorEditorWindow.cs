...
public class AvatarCreatureGeneratorEditorWindow : EditorWindow
{
    ...
    bool IsAllowedToGenerate()
    {
        return (
            ...
            selectedAnimator.avatar != null &&
            selectedAnimator.avatar.isHuman == true &&
            GetIsPathToGameDirectoryValid(absolutePathToGameDirectory) &&
            (
                (useForLocalToo == false && steamId.ToString().Length == 17) ||
                useForLocalToo == true
            )
        );
    }
    ...
    void OnGUI()
    {
        ...
        if (
            ...
            (
                selectedAnimator.avatar == null ||
                selectedAnimator.avatar.isHuman == false ||
                (
                    selectedAnimator.GetBoneTransform(HumanBodyBones.Chest) == null &&
                    selectedAnimator.GetBoneTransform(HumanBodyBones.Spine) == null
                )
            ))
        {
            CustomGUI.SmallLineGap();
            CustomGUI.RenderErrorMessage("ERROR: The selected animator is missing a avatar or it is not humanoid or it is missing a chest or spine bone.");
        }

        CustomGUI.SmallLineGap();

        CustomGUI.LargeLabel("Step 2: Options");

        CustomGUI.SmallLineGap();

        useForLocalToo = CustomGUI.Checkbox("Use for local player", useForLocalToo);
        CustomGUI.ItalicLabel("Only enable this if you use a 3rd person mod as your steam ID is 0.");

        CustomGUI.SmallLineGap();

        useAsDefault = CustomGUI.Checkbox("Use this avatar as default", useAsDefault);
        CustomGUI.ItalicLabel("If a player is missing an avatar the mod will search for 'default.assetbundle'");

        CustomGUI.SmallLineGap();

        CustomGUI.LargeLabel("Step 3: Paste your 17 digit Steam ID");

        CustomGUI.SmallLineGap();

        if (useAsDefault)
        {
            CustomGUI.ItalicLabel("Skipped because it is a default avatar");
        }
        else
        {
    ...
    {
        Texture texture = material.GetTextureSafe(textureNameInMaterial);

        if (texture is null)
        {
            Debug.Log($"Skipping Texture in material name: {textureNameInMaterial} since it doesn't exist.");
            return;
        }
    ...
    void Generate()
    {
        ...
        string assetBundleName = useAsDefault ? "default" : steamId.ToString();
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            {
                var pathToModelFile = AssetDatabase.GetAssetPath(prefab.GetComponent<SkinnedMeshRenderer>()?.sharedMesh);
            ...
            foreach (Material material in renderer.sharedMaterials)
            {
                ...
                {
                    if (material.GetTextureSafe("_MainTex") == null)
                    {
                        throw new System.Exception($"Material '{pathToMaterial}' is missing albedo");
                    }

                    try
                    {
                        AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(material.GetTextureSafe("_MainTex")),
                            $"{rendererName}_{idx}_Albedo.png");
                        AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(material.GetTextureSafe("_GlossMap")),
                            $"{rendererName}_{idx}_Smoothness.png");
                    }
                    catch
                    {
                        // eat excption intentionally
                    }

                    string pathToAlbedo = AssetDatabase.GetAssetPath(material.GetTextureSafe("_MainTex"));
                    string pathToEmission = AssetDatabase.GetAssetPath(material.GetTextureSafe("_EmissionMap"));
                    ...
                    {
                        AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(material.GetTextureSafe("_EmissionMap")), $"{rendererName}_{idx}_Emission.png");
                    }

                    AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(material.GetTextureSafe("_BumpMap")), $"{rendererName}_{idx}_Normal.png");
...
public static class Extensions
{
    ...
    public static Texture GetTextureSafe(this Material material, string name)
    {
        ...
        Debug.Log($"Material {material.name} does not have a {name} texture.");
